...
class Pipeline(BaseModel):
    ...
    name: str
    trigger: SerializeAsAny[TriggerConfig]
    worker: List[SerializeAsAny[WorkerConfig]] = []
    ...
    def ser_model(self) -> dict[str, Any]:
        return {
            "id": self.id,
            "name": self.name,
            "trigger": self.trigger.ser_model(),
            "worker": [workers_data.ser_model() for workers_data in self.worker]
        }
    ...
    def validate_trigger(cls, values: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not trigger_data:
            raise ValueError("Trigger data is required")
