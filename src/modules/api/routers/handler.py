...
from api.models.trigger import TriggerConfig
...
async def handle_job(pipeline_id: str, container_id: str, payload: JobPayload):
    ...
    try:
        ...
        if str(pipeline.trigger.id) == container_id:
            # Start the Trigger
            worker_handler.start_trigger(pipeline_id, pipeline.trigger, payload)
        else:
            # Stop the corresponding Worker container
            #worker = next((w for w in pipeline.worker if str(w.id) == container_id), None)
            #if worker:
            worker_handler.stop_worker(container_id)
